...
export default function RegisterPage() {
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    if (password !== String(form.get("confirmPassword") ?? "")) return setError("Mật khẩu xác nhận chưa khớp.");
    setSubmitting(true);
    const response = await fetch("/api/auth/register", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ username: form.get("username"), displayName: form.get("displayName"), facebookUrl: form.get("facebookUrl"), password }),
    });
  ...
  return <main className="paper-grid grid min-h-[100svh] place-items-center px-5 py-8">
    <div className="w-full max-w-[460px]">
      <Link href="/login" className="mb-4 inline-flex items-center gap-2 text-xs font-bold text-muted-foreground"><ArrowLeft className="size-3.5" />Đăng nhập</Link>
      <Card><CardContent className="p-6 sm:p-7">
        ...
        <form onSubmit={handleSubmit} onChange={() => error && setError("")} className="mt-4 flex flex-col gap-3">
          ...
          <label className="flex flex-col gap-1.5 text-xs font-bold">Xác nhận mật khẩu<Input className="text-base sm:text-sm" name="confirmPassword" type="password" required minLength={8} autoComplete="new-password" /></label>
          {error && <p role="alert" className="text-xs font-semibold text-destructive">{error}</p>}
          <Button type="submit" className="h-11 w-full" disabled={submitting}>{submitting ? "Đang tạo..." : "Tạo tài khoản"}<ArrowRight className="size-4" /></Button>
        </form>
      </CardContent></Card>
    </div>
  </main>;
