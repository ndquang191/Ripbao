  ...
  try {
    ...
    for (const row of rows) {
      ...
      const collection = collections.get(username) ?? {
        ...
        tradingLocations: Array.isArray(row.tradingLocations)
          ? row.tradingLocations.map(String)
          : [],
        cards: [],
      };
                ...
                <CardContent className="p-0">
                  <div className="flex items-center gap-3 border-b bg-card px-4 py-3">
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <p className="mt-0.5 flex min-w-0 items-center gap-1 truncate text-[10px] text-muted-foreground">
                        <MapPin className="size-3 shrink-0" />
                        <span className="truncate">
                          {collection.tradingLocations.length > 0
                            ? collection.tradingLocations.join(" · ")
                            : "Chưa cập nhật địa điểm giao dịch"}
                        </span>
                      </p>
                    </div>
                    <Link
