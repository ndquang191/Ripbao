...
export default function TradesPage() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    setLoadError("");
    const canReadGuestTrades = sessionState === "anonymous" || sessionState === "guest";
    const localTrades = canReadGuestTrades ? readGuestTrades() : [];
    try {
      const response = await fetch("/api/trades");
      if (response.status === 401) {
        if (localTrades.length > 0) { setTrades(localTrades); return; }
        window.location.href = "/login?next=/trades";
        return;
      }
      const data = await response.json().catch(() => null) as { requests?: Trade[]; error?: string } | null;
      if (!response.ok) throw new Error(data?.error ?? "Không thể tải danh sách giao dịch.");
      ...
      setTrades([...remoteTrades, ...localTrades.filter((trade) => !remoteIds.has(trade.id))]);
    } catch (error) {
      if (localTrades.length > 0) setTrades(localTrades);
      else setLoadError(error instanceof Error ? error.message : "Không thể tải danh sách giao dịch.");
    } finally {
      setLoading(false);
    }
  }, [sessionState]);
    ...
    try {
      if (action === "complete") {
        ...
        if (!saveResponse.ok) {
          ...
          toast({ title: saveData.error ?? "Không thể lưu số lượng trước khi đánh dấu đã giao dịch.", variant: "error" }); return;
        }
      }
      ...
      toast({ title: "Không thể kết nối để cập nhật giao dịch.", variant: "error" });
    } finally {
      setBusy(null);
    }
  };
  ...
  const deleteTrade = async (trade: Trade) => {
    if (!window.confirm("Ẩn giao dịch này khỏi danh sách của bạn? Phía còn lại vẫn có thể xem giao dịch.")) return;
    setBusy(`${trade.id}:delete`);
    try {
      const response = await fetch(`/api/trades/${trade.id}`, { method: "DELETE" });
      const data = await response.json().catch(() => ({})) as { error?: string };
      if (!response.ok) toast({ title: data.error ?? "Không thể ẩn giao dịch.", variant: "error" });
      else {
        removeGuestTrade(trade.id);
        setTrades((current) => current.filter((item) => item.id !== trade.id));
        toast({ title: "Đã ẩn giao dịch khỏi danh sách của bạn.", variant: "success" });
      }
    } catch {
  ...
  return <main className="paper-grid min-h-dvh"><div className="mx-auto w-full max-w-6xl px-5 py-5 sm:px-8 lg:py-7">
    <header className="flex items-center justify-between border-b pb-5"><Link href="/" className="flex items-center gap-3 text-sm font-extrabold tracking-[0.16em]"><BrandLogo /> RIPBAO</Link><AccountLink /></header>
    ...
    {loadError ? <Card className="grid min-h-64 place-items-center border-dashed bg-card/70 p-8 text-center"><div><XCircle className="mx-auto size-8 text-destructive"/><h2 className="mt-3 font-serif text-lg font-semibold">Không thể tải giao dịch</h2><p className="mt-1 text-xs text-muted-foreground">{loadError}</p><Button className="mt-4" size="sm" onClick={() => void load()}>Thử lại</Button></div></Card> : trades.length === 0 ? <Card className="grid min-h-64 place-items-center border-dashed bg-card/70 p-8 text-center"><div><Handshake className="mx-auto size-8 text-muted-foreground"/><h2 className="mt-3 font-serif text-lg font-semibold">Chưa có yêu cầu giao dịch</h2><p className="mt-1 text-xs text-muted-foreground">Chọn card trong collection của người khác và gửi từ giỏ hàng.</p></div></Card> : <div className="space-y-4">{trades.map((trade) => {
      ...
      return <Card key={trade.id} className="overflow-hidden"><div className="flex flex-wrap items-center justify-between gap-2 border-b bg-card/80 px-4 py-3"><div><span className="text-[9px] font-bold tracking-wider text-muted-foreground uppercase">{isSeller ? "Yêu cầu mua từ" : "Gửi tới"}</span><div className="flex items-center gap-1.5">{trade.counterpartyIsGuest ? <strong>{other}</strong> : <Link href={`/u/${trade.counterpartyUsername}`} className="font-bold hover:underline">{other}</Link>}{facebookUrl && <a href={facebookUrl} target="_blank" rel="noreferrer" className="grid size-6 place-items-center rounded-sm border bg-card text-[10px] font-black text-primary transition-colors hover:bg-secondary" aria-label={`Mở Facebook của ${other}`} title="Mở Facebook">f</a>}</div></div><span className={`rounded-full px-2.5 py-1 text-[9px] font-extrabold ${trade.status === "completed" ? "bg-[#dce9ca] text-[#426026]" : trade.status === "cancelled" ? "bg-red-100 text-red-700" : "bg-amber-100 text-amber-800"}`}>{statusLabel[trade.status]}</span></div>
