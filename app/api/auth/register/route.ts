...
export async function POST(request: Request) {
  ...
  if (!validUsername(username))
    return NextResponse.json(
      {
        error:
          "Username phải dài 3–32 ký tự và chỉ gồm chữ thường, số, dấu chấm hoặc gạch dưới.",
      },
      { status: 400 },
    );
  if (!displayName)
  ...
  try {
    ...
    if (currentUser?.isGuest) {
      await sql.transaction((tx) => [
        ...
        ...defaultTradingOptions.map(
          ([optionId, name, position]) => tx`
      ...
      return NextResponse.json(
        { user: { id: currentUser.id, username, displayName, facebookUrl } },
        { status: 201 },
      );
    }
    ...
    return NextResponse.json(
      { user: { id: userId, username, displayName, facebookUrl } },
      { status: 201 },
    );
  } catch (error) {
