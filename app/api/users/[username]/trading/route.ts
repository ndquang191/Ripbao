...
type TradingPreference = { id?: string; optionId?: string | null; name?: string; detail?: string; enabled?: boolean; position?: number };
...
export async function PUT(request: Request, { params }: { params: Promise<{ username: string }> }) {
  ...
  const username = normalizeUsername(decodeURIComponent((await params).username));
  if (!user || user.username !== username) return NextResponse.json({ error: "Bạn không có quyền cập nhật." }, { status: 403 });
  const body = await request.json().catch(() => null) as { items?: TradingPreference[] } | null;
  const items = (body?.items ?? []).slice(0, 30).map((item, index) => ({
    optionId: item.optionId || null,
    name: String(item.name ?? "").trim(),
    detail: String(item.detail ?? "").trim() || null,
    enabled: item.enabled === true,
    position: Number.isInteger(item.position) && Number(item.position) >= 0 ? Number(item.position) : index,
