...
type TradeOptionId =
  "nexus-night" | "skirmish" | "ship-inner-city" | "ship-nationwide";
type TradingData = {
  locations: TradingLocation[];
  selectedOptions: TradeOptionId[];
};
type TradingApiItem = {
  id: string;
  ...
  const request = fetch(`/api/users/${encodeURIComponent(username)}/trading`)
    ...
    .then((data: { items?: TradingApiItem[] }) => {
      ...
      const result: TradingData = {
        locations: items
          .filter((item) => item.optionId === null && item.enabled)
          .map((item) => ({
            id: item.id,
            name: item.name,
            detail: item.detail ?? "",
        ...
        selectedOptions: items
          .filter((item) => item.optionId && item.enabled)
          .map((item) => item.optionId as TradeOptionId)
...
export function TradingLocations({ username }: { username: string }) {
  ...
  const [selectedOptions, setSelectedOptions] = useState<TradeOptionId[]>(
  ...
  useEffect(() => {
    ...
    loadTrading(normalizedUsername)
      .then((data) => {
        if (!active) return;
  ...
  function savePreferences(
    nextLocations: TradingLocation[],
    nextOptions: TradeOptionId[],
  ) {
    tradingCache.set(normalizedUsername, {
      locations: nextLocations,
      selectedOptions: nextOptions,
      cachedAt: Date.now(),
    });
    const items = [
      ...tradeOptions.map((option, position) => ({
        optionId: option.id,
        name: option.label,
        enabled: nextOptions.includes(option.id),
        position,
      })),
    ...
    void fetch(`/api/users/${encodeURIComponent(normalizedUsername)}/trading`, {
      method: "PUT",
  ...
  function addLocation(event: FormEvent<HTMLFormElement>) {
    ...
    const nextLocations = [
      ...locations,
      { id: crypto.randomUUID(), name, detail },
    ];
  ...
  function toggleOption(id: TradeOptionId) {
    const nextOptions = selectedOptions.includes(id)
  ...
  const visibleOptions =
    isOwner && isEditing
      ? tradeOptions
      : tradeOptions.filter((option) => selectedOptions.includes(option.id));
  ...
  return (
    <section
      ...
      aria-labelledby="trading-locations-title"
    >
      <div className="mb-3">
        <h2
          id="trading-locations-title"
          className="flex items-center gap-2 font-serif text-lg font-semibold text-[#5f793f]"
        >
      ...
      <div className="relative flex min-w-0 items-center gap-2 overflow-hidden rounded-sm border bg-card/75 px-3 py-2">
        {locations.length > 0 || visibleOptions.length > 0 ? (
          <ul
            ...
            className={cn(
              "flex min-w-0 flex-1 flex-nowrap gap-1.5 overflow-x-auto whitespace-nowrap [scrollbar-width:none] [&::-webkit-scrollbar]:hidden",
              isOwner && "pr-10",
            )}
          >
            {locations.map((location) => (
              <li
                ...
                className="relative flex h-7 shrink-0 items-center gap-1.5 overflow-hidden whitespace-nowrap rounded-sm border bg-secondary py-0 pr-7 pl-2.5 text-[9px]"
              >
                ...
                {isOwner && isEditing && (
                  <button
                    ...
                    className="absolute top-1/2 right-2 -translate-y-1/2 text-muted-foreground transition-colors hover:text-destructive"
            ...
            {visibleOptions.map((option) => {
              ...
              return (
                <li key={option.id}>
                  <label
                    className={cn(
                      "flex h-7 shrink-0 items-center gap-1.5 rounded-sm border bg-secondary px-2.5 text-[9px] font-bold",
                      isOwner &&
                        isEditing &&
                        "cursor-pointer hover:border-[#8ba55e]",
                      !checked && "text-muted-foreground opacity-65",
                    )}
                  >
                    {isOwner && isEditing ? (
                      <input
