...
function CardArtwork({ featuredCard }: { featuredCard: FeaturedCard | null }) {
  return (
    <div
      ...
      aria-label={
        featuredCard
          ? `Card đắt nhất hiện có: ${featuredCard.name}`
          : "Minh hoạ thẻ bài Riftbound"
      }
    >
      ...
      <div className="absolute top-10 left-1/2 h-[322px] w-[222px] -translate-x-[22%] rotate-[12deg] rounded-lg border-4 border-[#76966e] bg-primary shadow-2xl">
        <span className="grid h-full place-items-center font-serif text-8xl text-accent/35">
          R
        </span>
      </div>
      ...
      <Card className="absolute right-0 bottom-12 z-20 min-w-[220px] border-l-[3px] border-l-accent shadow-xl">
        <CardContent className="p-3.5">
          <span className="block text-[8px] font-bold tracking-wider text-muted-foreground uppercase">
            Bạn có rất nhiều tiền ?
          </span>
          <div className="mt-0.5 flex items-end justify-between gap-3">
            <strong className="whitespace-nowrap font-serif text-2xl">
              {featuredCard
                ? formatCurrency(featuredCard.price)
                : "Chưa có giá"}
            </strong>
            {featuredCard && (
              <b className="shrink-0 pb-1 text-[9px] text-[#65982e]">
                {featuredCard.tcgPercent}% TCG
              </b>
            )}
          </div>
        </CardContent>
      </Card>
      <span className="absolute top-16 right-0 text-3xl text-[#90a967]">✦</span>
      <span className="absolute bottom-8 left-14 text-xl text-[#90a967]">
        ✧
      </span>
    </div>
  );
...
export default async function Home() {
  const [sellers, featuredCard] = await Promise.all([
    loadSellers(),
    loadFeaturedCard(),
  ]);
  return (
          ...
          <div>
            <form action="/search" className="relative mb-5" role="search">
              ...
              <Input
                name="q"
                required
                className="h-11 bg-card/95 pr-23 pl-10 shadow-sm"
              ...
              <button
                ...
                className={cn(
                  ...
                  "absolute top-1.5 right-1.5 h-8",
                )}
              >
                Tìm
              </button>
            ...
            <div className="mb-4 flex items-end justify-between">
              ...
              <span className="hidden text-xs text-muted-foreground sm:block">
                {sellers.length} collection đang bán
              </span>
            </div>
            <div className="grid gap-2.5">
              {sellers.map((seller, index) => (
                ...
                >
                  <Card
                    className={cn(
                      "transition-all group-hover:translate-x-1 group-hover:border-[#8ba55e] group-hover:shadow-md",
                      index === 0 &&
                      ...
                      <div className="min-w-0 flex-1">
                        ...
                        <p
                          className={cn(
                            ...
                            index === 0 && "text-white/60",
                          )}
                        >
                          {seller.cards} card · {seller.rare} card hiếm
                        </p>
                      </div>
                      <ArrowUpRight
                        className={cn(
                ...
                <Card className="border-dashed bg-card/70">
                  <CardContent className="p-6 text-center">
                    <p className="font-serif text-lg font-semibold">
                      Chưa có collection đang bán
                    </p>
                    <p className="mt-1 text-xs text-muted-foreground">
                      Hãy trở thành người đầu tiên đăng collection trên Ripbao.
                    </p>
                  </CardContent>
                </Card>
              )}
