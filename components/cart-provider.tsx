...
export function CartProvider({ children }: { children: React.ReactNode }) {
  ...
  const [sessionState, setSessionState] = useState<CartContextValue["sessionState"]>("anonymous");
  const [saveStatus, setSaveStatus] = useState<CartContextValue["saveStatus"]>("idle");
  ...
  useEffect(() => {
    ...
    const cartRequest = fetch("/api/cart").then(async (response) => {
      ...
      const data = await response.json() as { items?: Array<{ cardId: string; seller: string; sellerDisplayName: string; sellerFacebookUrl?: string; name: string; set: string; number: number; finish: string; condition: string; unitPrice: number; imageUrl: string; stock: number; quantity: number }> };
      setItems((data.items ?? []).map((item) => ({ key: `${item.seller}:${item.cardId}`, cardId: item.cardId, seller: item.seller, sellerDisplayName: item.sellerDisplayName, sellerFacebookUrl: item.sellerFacebookUrl, name: item.name, set: item.set, number: String(item.number).padStart(3, "0"), finish: item.finish === "foil" ? "Foil" : "No Foil", condition: item.condition, price: Number(item.unitPrice) > 0 ? `${Number(item.unitPrice).toLocaleString("vi-VN")} ₫` : "Liên hệ", imageUrl: item.imageUrl, glyph: "R", gradient: "from-[#91c6bd] via-[#477a78] to-[#283d54]", stock: Number(item.stock), quantity: Number(item.quantity) })));
      setRemote(true);
    }).catch(() => {
    ...
    const sessionRequest = fetch("/api/auth/session")
      ...
      .then((data: { user?: SessionUser | null }) => {
        ...
        setSessionUser(user);
        setSessionState(user ? (user.isGuest ? "guest" : "authenticated") : "anonymous");
      })
      .catch(() => {
        setSessionUser(null);
    ...
    if (remote) {
      ...
      const timer = window.setTimeout(() => {
        ...
        saveQueue.current = saveQueue.current.catch(() => undefined).then(async () => {
          const response = await fetch("/api/cart", { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ items: snapshot }) });
