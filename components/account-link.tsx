...
export function AccountLink({ className }: { className?: string }) {
  ...
  const [facebookUrl, setFacebookUrl] = useState(sessionUser?.facebookUrl ?? "");
  ...
  useEffect(() => {
    const closeForOtherDropdown = (event: Event) => {
      if ((event as CustomEvent<string>).detail !== accountMenuId) setAccountMenuOpen(false);
    };
  ...
  useEffect(() => {
    if (!username) { setPendingTrades(0); return; }
    fetch("/api/trades").then((response) => response.ok ? response.json() : Promise.reject()).then((data: { requests?: Array<{ role: string; status: string }> }) => setPendingTrades((data.requests ?? []).filter((trade) => trade.role === "seller" && trade.status === "pending").length)).catch(() => setPendingTrades(0));
  }, [username]);
  ...
  const saveProfile = async (event: FormEvent<HTMLFormElement>) => {
    ...
    if (!username) return;
    const response = await fetch("/api/profile", { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ displayName, facebookUrl, password }) });
    const data = await response.json() as { error?: string };
    if (!response.ok) return toast({ title: data.error ?? "Không thể lưu thông tin.", variant: "error" });
        ...
        <div className="relative flex h-8 items-center">
          ...
          {accountMenuOpen && <>
            ...
            <div className="absolute right-0 top-full z-50 mt-2 w-44 rounded-md border bg-card p-1.5 shadow-xl" role="menu">
              <Link href="/collection" onClick={() => setAccountMenuOpen(false)} className="flex h-9 items-center gap-2 rounded-sm px-2.5 text-xs font-bold hover:bg-secondary" role="menuitem"><LibraryBig className="size-3.5" />Collection</Link>
              <button type="button" onClick={openProfile} className="flex h-9 w-full items-center gap-2 rounded-sm px-2.5 text-xs font-bold hover:bg-secondary" role="menuitem"><Settings className="size-3.5" />Cài đặt</button>
              <div className="my-1 border-t" />
              <button type="button" onClick={() => { setAccountMenuOpen(false); setLogoutConfirmOpen(true); }} className="flex h-9 w-full items-center gap-2 rounded-sm px-2.5 text-xs font-bold text-destructive hover:bg-destructive/10" role="menuitem"><LogOut className="size-3.5" />Đăng xuất</button>
            </div>
          </>}
        </div>
      </div>

      {logoutConfirmOpen && <div className="fixed inset-0 z-50 grid place-items-center p-4" role="dialog" aria-modal="true" aria-labelledby="logout-confirm-title">
        <button type="button" className="absolute inset-0 bg-primary/35 backdrop-blur-[1px]" onClick={() => setLogoutConfirmOpen(false)} aria-label="Huỷ đăng xuất" />
        <div className="relative w-full max-w-sm rounded-md border bg-card p-5 shadow-2xl">
          <h2 id="logout-confirm-title" className="font-serif text-lg font-semibold">Xác nhận đăng xuất</h2>
          <p className="mt-2 text-sm text-muted-foreground">Bạn có chắc muốn đăng xuất khỏi Ripbao?</p>
          <div className="mt-5 flex justify-end gap-2">
            <Button type="button" variant="outline" size="sm" onClick={() => setLogoutConfirmOpen(false)}>Huỷ</Button>
            <Button type="button" size="sm" className="bg-destructive text-white hover:bg-destructive/85" onClick={logout}><LogOut className="size-3.5" />Đăng xuất</Button>
          </div>
        </div>
      </div>}
      ...
      {profileOpen && <div className="fixed inset-0 z-50" role="dialog" aria-modal="true" aria-labelledby="profile-drawer-title">
        ...
        <aside className="absolute inset-y-0 left-0 flex w-[min(92vw,420px)] flex-col border-r bg-card shadow-2xl">
          ...
          <form className="flex flex-1 flex-col overflow-y-auto p-5 sm:p-6" onSubmit={saveProfile}>
            ...
            <div className="mt-auto pt-6"><Button type="submit" className="w-full">Lưu thay đổi</Button></div>
          </form>
        </aside>
      </div>}
    </>
